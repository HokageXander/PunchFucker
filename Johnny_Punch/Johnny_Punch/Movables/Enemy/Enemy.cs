...
namespace Johnny_Punch
{
    abstract class Enemy : Movables
    {
        ...
        {
            spriteBatch.Draw(TextureManager.playerShadow, new Vector2(pos.X, pos.Y + (height / 2)), null, new Color(0, 0, 0, 120), 0f, new Vector2(63 / 2, 21 / 2), 1, SpriteEffects.None, 0);
        ...
        {
            angryFace = false;
            if (Vector2.Distance(pos, player.pos) < aggroRadius && Vector2.Distance(pos, player.pos) > 90 /* && står i samma Y-led */)
            {
            ...
            if(Vector2.Distance(pos, player.pos) < 90)
            {
